        ...
        return (
          <div key={devto.id} className='flex flex-col bg-gray-800 shadow-sm p-2 rounded-lg max-w-md mx-3 my-4 flex-wrap justify-between'>
            ...
            <div className='flex items-center w-10/12 justify-between my-3 mx-auto'>
              ...
              <img className='rounded-full w-12 flex items-center justify-center' src={devto.user.profile_image_90} />
            </div>
            <div className='flex justify-evenly w-10/12 mx-auto flex-wrap'>
              {devto.tag_list.map((tag) => (
                <div className='inline-flex justify-between items-center px-3 font-medium text-white shadow-md rounded-full bg-purple-600 text-s mt-1'>{tag}</div>
              ))}
            </div>
            <div className='flex justify-evenly w-10/12 mx-auto py-2'>
              ...
              <div className='text-sm font-bold text-purple-300'>
                Reactions: {devto.public_reactions_count} {devto.public_reactions_count >= 20 ? '🔥' : ''}
              </div>
            </div>
          </div>
        );
